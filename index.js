...
const total_contribution = GAMES_JSON.reduce((total, games)=>{
...
const total_raised = GAMES_JSON.reduce((total, games)=>{
...
function filterUnfundedOnly() { 
    ...
    let unreachGoal = GAMES_JSON.filter((games)=>{
        return games.goal > games.pledged;
    });


    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(unreachGoal);
...
function filterFundedOnly() {
    ...
    let reachGoal = GAMES_JSON.filter((games)=>{
        return games.pledged >= games.goal;
    });

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(reachGoal);
}
...
const unfundedGamesText = unfundedGames === 1 
    ? `There is 1 game that remains unfunded.` 
    : `A total of $${total_raised} has been raised for ${totalNumberOfGames} games. There are ${unfundedGames} games that remain unfunded currently. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const companyInfoElement = document.createElement('p');
companyInfoElement.innerHTML = unfundedGamesText;
